...
solution = solve(prob, DP5(), abstol=1e-12, reltol=1e-12, saveat = 0.1)
...
@variables u[1:4]
# Lots of polynomials
order = 0 # order soll sein: Summe aller Exponenten in jedem Monom
polys = Operation[1]
for i ∈ 1:order
    push!(polys, u[1]^i)
    push!(polys, u[2]^i)
...
a_solution = solve(a_prob, DP5(), saveat = 0.1)
